...
public class BindingKey {
     ...
     * 
     * @param typeKey
     *            the binding key of the given type
    ...
    /**
     * Creates a new parameterized type binding key from the given generic type binding key and the given argument type
     * binding keys. If the argument type keys array is empty, then a raw type binding key is created.
     * <p>
     * For example:
     * 
     * <pre>
     ...
     * </p>
     * 
     * @param genericTypeKey
     *            the binding key of the generic type
     * @param argumentTypeKeys
    ...
    public static String createParameterizedTypeBindingKey(String genericTypeKey, String[] argumentTypeKeys) {
        // Note this implementation is heavily dependent on ParameterizedTypeBinding#computeUniqueKey() and its
        // subclasses
        StringBuffer buffer= new StringBuffer();
    ...
    /**
     * Creates a new type binding key from the given type name. The type name must be either a fully qualified name, an
     * array type name or a primitive type name. If the type name is fully qualified, then it is expected to be
     * dot-based. Note that inner types, generic types and parameterized types are not supported.
     * <p>
     * For example:
     * 
     * <pre>
    ...
    /**
     * Creates a new type variable binding key from the given type variable name and the given declaring key. The
     * declaring key can either be a type binding key or a method binding key.
     * <p>
     * For example:
     * 
     * <pre>
     ...
     *            the name of the given type variable
     * @param declaringKey
    ...
    /**
     * Creates a new wildcard type binding key from the given type binding key and the given wildcard kind (one of
     * {@link Signature#C_STAR}, {@link Signature#C_SUPER}, or {@link Signature#C_EXTENDS}. If the wildcard is
     * {@link Signature#C_STAR}, the given type binding key is ignored.
     * <p>
     * For example:
     * 
     * <pre>
